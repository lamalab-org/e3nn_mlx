...
| `linear_test.py` | equivariance/compile/normalization, per-output biases, disconnected outputs, exact TensorProduct equivalence, masks, explicit/default/empty instructions, upstream default path order, shared and unshared views, feature channels, grouped external-weight VJPs | `tests/test_upstream_o3_linear_norm.py`, `tests/test_mlx_module_state.py`, `tests/test_mlx_module_integration.py` |
