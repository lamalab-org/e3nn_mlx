...
class IrrepsTest(unittest.TestCase):
    ...
    def test_parse_irreps(self) -> None:
        ...
        self.assertEqual(str(irreps), "2x0e+1x1o+3x2e")
    ...
    def test_regroup_and_sort(self) -> None:
        ...
        self.assertEqual(str(sorted_irreps.irreps), "2x0e+1x1o+3x1o")
    ...
    def test_simplify_merges_only_adjacent_irreps(self) -> None:
        self.assertEqual(str(Irreps("1e+1e+0e+1e").simplify()), "2x1e+1x0e+1x1e")

    def test_y_parity_and_spherical_harmonics(self) -> None:
        self.assertEqual(Irrep("1y"), Irrep("1o"))
        self.assertEqual(Irrep("2y"), Irrep("2e"))
        self.assertEqual(str(Irreps.spherical_harmonics(3)), "1x0e+1x1o+1x2e+1x3o")

    def test_filter_and_remove_zeros(self) -> None:
        irreps = Irreps("0e+0x2e+2x1o+3e")
        self.assertEqual(irreps.lmax, 3)
        self.assertEqual(str(irreps.remove_zero_multiplicities()), "1x0e+2x1o+1x3e")
        self.assertEqual(str(irreps.filter(lmax=1)), "1x0e+2x1o")
        self.assertEqual(str(irreps.filter(keep=["1o", "3e"])), "2x1o+1x3e")
